...
function transformPetResponse(pet: Pet) {
  if (pet.birthDate !== null && pet.birthDate !== undefined) pet.birthDate = new Date(pet.birthDate);
}
function transformOrderResponse(order: Order) {
  if (order.shipDate !== undefined) order.shipDate = new Date(order.shipDate);
  if (order.pets !== null && order.pets !== undefined)
